/***
   ...
   *  @description
	*
	*	实验平台：反客STM32H743XIH6核心板 （型号：FK743M5-XIH6）+ 4.3寸RGB液晶屏(屏幕型号RGB043M1-480*272)
...
void LCD_Test_DoubleLayer(void)
{
	...
	LCD_DrawLine(45, 60, 300, 60);
	HAL_Delay(time);
	LCD_SetColor(LIGHT_MAGENTA);
	LCD_DrawLine(45, 90, 300, 90);
	HAL_Delay(time);
	LCD_SetColor(LIGHT_YELLOW);
	LCD_DrawLine(45, 120, 300, 120);
	HAL_Delay(time);
	LCD_SetColor(LIGHT_GREY);
	LCD_DrawLine(45, 150, 300, 150);
	HAL_Delay(time);
	LCD_SetColor(LIGHT_CYAN);
	LCD_DrawLine(45, 180, 300, 180);
	HAL_Delay(time);
	LCD_SetColor(LIGHT_MAGENTA);
	LCD_DrawLine(45, 210, 300, 210);
	HAL_Delay(time);
	LCD_SetColor(LIGHT_YELLOW);
	LCD_DrawLine(45, 240, 300, 240);
	HAL_Delay(time);

	LCD_SetColor(LCD_RED);
	LCD_FillCircle(118, 350, 85);
	HAL_Delay(time);
	LCD_SetColor(LCD_GREEN);
	LCD_FillCircle(187, 350, 85);
	HAL_Delay(time);
	LCD_SetColor(LCD_BLUE);
	LCD_FillCircle(268, 350, 85);
	HAL_Delay(time);

	LCD_SetColor(LIGHT_RED);
	LCD_FillRect(387, 53, 200, 200);
	HAL_Delay(time);
	LCD_SetColor(LIGHT_GREEN);
	LCD_FillRect(477, 141, 200, 200);
	HAL_Delay(time);
	LCD_SetColor(LIGHT_BLUE);
	LCD_FillRect(581, 248, 200, 200);
	HAL_Delay(time);
	...
	LCD_SetColor(LCD_WHITE);				  // 画笔不透明
	LCD_DisplayText(110, 140, "STM32H743 LTDC, ARGB4444, layer1");
	HAL_Delay(1000);

	LCD_SetBackColor(LCD_BLACK & 0x80FFFFFF); // 设置背景色透明度，高8位的数字越小透明度越高，ARGB4444支持4位透明色，有16种透明状态
	LCD_SetColor(LCD_WHITE);
	LCD_DisplayText(110, 200, "STM32H743 LTDC, ARGB4444, layer1");
	HAL_Delay(1000);

	LCD_SetBackColor(LCD_BLACK & 0x30FFFFFF); // 设置背景色透明度，高8位的数字越小透明度越高，ARGB4444支持4位透明色，有16种透明状态
	LCD_SetColor(LCD_WHITE);
	LCD_DisplayText(110, 260, "STM32H743 LTDC, ARGB4444, layer1");
	HAL_Delay(1000);

	LCD_SetBackColor(LCD_BLACK);		  // 将背景色设置为 不透明
	LCD_SetColor(LCD_WHITE & 0x00FFFFFF); // 将画笔色设置为 完全透明
	LCD_DisplayText(110, 320, "STM32H743 LTDC, ARGB4444, layer1");
	HAL_Delay(2000);
	...
	LCD_SetColor(LCD_WHITE);				 // 画笔不透明
	LCD_DisplayText(110, 140, "STM32H743 LTDC, ARGB1555, layer1");
	HAL_Delay(1000);

	LCD_SetBackColor(LCD_BLACK & 0x00FFFFFF); // 设置背景色透明度，高8位的数字越小透明度越高，ARGB1555仅支持一位透明色，即仅有透明和不透明两种状态
	LCD_SetColor(LCD_WHITE);				  // 画笔不透明
	LCD_DisplayText(110, 200, "STM32H743 LTDC, ARGB1555, layer1");
	HAL_Delay(1000);

	LCD_SetBackColor(LCD_BLACK & 0xffFFFFF); // 将背景色设置为 不透明
	LCD_SetColor(LCD_WHITE & 0x00FFFFFF);	 // 将画笔色设置为 完全透明
	LCD_DisplayText(110, 260, "STM32H743 LTDC, ARGB1555, layer1");
	HAL_Delay(2000);
...
void Qinwei(int flower) //	?????? flower??????
{
	...
	if (S == 'A')
	{
		...
		LCD_DisplayText(10, 10 + 60 * (flower), "轻微干旱");
	}
	if (S == 'B')
	{
		LCD_SetTextFont(&CH_Font32);
		LCD_DisplayText(200, 10 + 60 * (flower - 6), "轻微干旱");
	}
	if (S == 'C')
	{
		LCD_SetTextFont(&CH_Font32);
		LCD_DisplayText(390, 10 + 60 * (flower - 12), "轻微干旱");
	}
	if (S == 'D')
	{
		LCD_SetTextFont(&CH_Font32);
		LCD_DisplayText(580, 10 + 60 * (flower - 18), "轻微干旱");
	}
}
...
void LCD_Test_FillRect(void)
{
	...
	LCD_SetFont(&Font16);
	LCD_SetColor(LCD_BLUE);
	LCD_FillRect(200, 3, 500, 16);
	LCD_DisplayString(0, 3, "LCD_BLUE");
	LCD_SetColor(LCD_GREEN);
	LCD_FillRect(200, 25, 500, 16);
	LCD_DisplayString(0, 25, "LCD_GREEN");
	LCD_SetColor(LCD_RED);
	LCD_FillRect(200, 47, 500, 16);
	LCD_DisplayString(0, 47, "LCD_RED");
	LCD_SetColor(LCD_CYAN);
	LCD_FillRect(200, 69, 500, 16);
	LCD_DisplayString(0, 69, "LCD_CYAN");
	LCD_SetColor(LCD_MAGENTA);
	LCD_FillRect(200, 91, 500, 16);
	LCD_DisplayString(0, 91, "LCD_MAGENTA");
	LCD_SetColor(LCD_YELLOW);
	LCD_FillRect(200, 113, 500, 16);
	LCD_DisplayString(0, 113, "LCD_YELLOW");
	LCD_SetColor(LCD_GREY);
	LCD_FillRect(200, 135, 500, 16);
	LCD_DisplayString(0, 135, "LCD_GREY");

	LCD_SetColor(LIGHT_BLUE);
	LCD_FillRect(200, 157, 500, 16);
	LCD_DisplayString(0, 157, "LIGHT_BLUE");
	LCD_SetColor(LIGHT_GREEN);
	LCD_FillRect(200, 179, 500, 16);
	LCD_DisplayString(0, 179, "LIGHT_GREEN");
	LCD_SetColor(LIGHT_RED);
	LCD_FillRect(200, 201, 500, 16);
	LCD_DisplayString(0, 201, "LIGHT_RED");
	LCD_SetColor(LIGHT_CYAN);
	LCD_FillRect(200, 223, 500, 16);
	LCD_DisplayString(0, 223, "LIGHT_CYAN");
	LCD_SetColor(LIGHT_MAGENTA);
	LCD_FillRect(200, 245, 500, 16);
	LCD_DisplayString(0, 245, "LIGHT_MAGENTA");
	LCD_SetColor(LIGHT_YELLOW);
	LCD_FillRect(200, 267, 500, 16);
	LCD_DisplayString(0, 267, "LIGHT_YELLOW");
	LCD_SetColor(LIGHT_GREY);
	LCD_FillRect(200, 289, 500, 16);
	LCD_DisplayString(0, 289, "LIGHT_GREY");

	LCD_SetColor(DARK_BLUE);
	LCD_FillRect(200, 311, 500, 16);
	LCD_DisplayString(0, 311, "DARK_BLUE");
	LCD_SetColor(DARK_GREEN);
	LCD_FillRect(200, 333, 500, 16);
	LCD_DisplayString(0, 333, "DARK_GREEN");
	LCD_SetColor(DARK_RED);
	LCD_FillRect(200, 355, 500, 16);
	LCD_DisplayString(0, 355, "DARK_RED");
	LCD_SetColor(DARK_CYAN);
	LCD_FillRect(200, 377, 500, 16);
	LCD_DisplayString(0, 377, "DARK_CYAN");
	LCD_SetColor(DARK_MAGENTA);
	LCD_FillRect(200, 399, 500, 16);
	LCD_DisplayString(0, 399, "DARK_MAGENTA");
	LCD_SetColor(DARK_YELLOW);
	LCD_FillRect(200, 421, 500, 16);
	LCD_DisplayString(0, 421, "DARK_YELLOW");
	LCD_SetColor(DARK_GREY);
	LCD_FillRect(200, 443, 500, 16);
...
void LCD_Test_Color(void)
{
	...
	LCD_DisplayText(42, 70, "核心板型号：FK743M5");
...
void LCD_Test_GrahicTest(void)
{
	...
	HAL_Delay(time);
	LCD_DrawEllipse(590, 350, 110, 40);
	HAL_Delay(time);
...
void LCD_Test_Vertical(void)
{
	...
	LCD_SetColor(LCD_WHITE);
	LCD_SetFont(&Font32);
	LCD_DisplayString(0, 0, "!#$%&'()*+,-.0123456789:;<=>");
	HAL_Delay(time);
	LCD_SetFont(&Font24);
	LCD_DisplayString(0, 32, "!#$%&'()*+,-.0123456789:;<=>");
	HAL_Delay(time);
	LCD_SetFont(&Font20);
	LCD_DisplayString(0, 56, "!#$%&'()*+,-.0123456789:;<=>");
	HAL_Delay(time);
	LCD_SetFont(&Font16);
	LCD_DisplayString(0, 76, "!#$%&'()*+,-.0123456789:;<=>");
	HAL_Delay(time);
	LCD_SetFont(&Font12);
	LCD_DisplayString(0, 92, "!#$%&'()*+,-.0123456789:;<=>");
	HAL_Delay(time);

	LCD_SetColor(LCD_CYAN);
	LCD_SetFont(&Font12);
	LCD_DisplayString(0, 104, "!#$%&'()*+,-.0123456789:;<=>");
	HAL_Delay(time);
	LCD_SetFont(&Font16);
	LCD_DisplayString(0, 116, "!#$%&'()*+,-.0123456789:;<=>");
	HAL_Delay(time);
	LCD_SetFont(&Font20);
	LCD_DisplayString(0, 132, "!#$%&'()*+,-.0123456789:;<=>");
	HAL_Delay(time);
	LCD_SetFont(&Font24);
	LCD_DisplayString(0, 152, "!#$%&'()*+,-.0123456789:;<=>");
	HAL_Delay(time);
	LCD_SetFont(&Font32);
	...
	LCD_DisplayText(40, 500 + 40, "核心板型号：FK743M5");
	...
	HAL_Delay(time);
	LCD_SetColor(LCD_MAGENTA);
	LCD_DrawCircle(200, 480, 80);
	HAL_Delay(time);
	LCD_SetColor(LCD_RED);
	LCD_DrawCircle(250, 480, 80);
	HAL_Delay(time);
	...
	HAL_Delay(time);
	LCD_DrawEllipse(200, 680, 110, 40);
	HAL_Delay(time);
	...
	LCD_SetFont(&Font16);
	LCD_SetColor(LCD_BLUE);
	LCD_FillRect(150, 3, 250, 16);
	LCD_DisplayString(0, 3, "LCD_BLUE");
	LCD_SetColor(LCD_GREEN);
	LCD_FillRect(150, 25, 250, 16);
	LCD_DisplayString(0, 25, "LCD_GREEN");
	LCD_SetColor(LCD_RED);
	LCD_FillRect(150, 47, 250, 16);
	LCD_DisplayString(0, 47, "LCD_RED");
	LCD_SetColor(LCD_CYAN);
	LCD_FillRect(150, 69, 250, 16);
	LCD_DisplayString(0, 69, "LCD_CYAN");
	LCD_SetColor(LCD_MAGENTA);
	LCD_FillRect(150, 91, 250, 16);
	LCD_DisplayString(0, 91, "LCD_MAGENTA");
	LCD_SetColor(LCD_YELLOW);
	LCD_FillRect(150, 113, 250, 16);
	LCD_DisplayString(0, 113, "LCD_YELLOW");
	LCD_SetColor(LCD_GREY);
	LCD_FillRect(150, 135, 250, 16);
	LCD_DisplayString(0, 135, "LCD_GREY");

	LCD_SetColor(LIGHT_BLUE);
	LCD_FillRect(150, 157, 250, 16);
	LCD_DisplayString(0, 157, "LIGHT_BLUE");
	LCD_SetColor(LIGHT_GREEN);
	LCD_FillRect(150, 179, 250, 16);
	LCD_DisplayString(0, 179, "LIGHT_GREEN");
	LCD_SetColor(LIGHT_RED);
	LCD_FillRect(150, 201, 250, 16);
	LCD_DisplayString(0, 201, "LIGHT_RED");
	LCD_SetColor(LIGHT_CYAN);
	LCD_FillRect(150, 223, 250, 16);
	LCD_DisplayString(0, 223, "LIGHT_CYAN");
	LCD_SetColor(LIGHT_MAGENTA);
	LCD_FillRect(150, 245, 250, 16);
	LCD_DisplayString(0, 245, "LIGHT_MAGENTA");
	LCD_SetColor(LIGHT_YELLOW);
	LCD_FillRect(150, 267, 250, 16);
	LCD_DisplayString(0, 267, "LIGHT_YELLOW");
	LCD_SetColor(LIGHT_GREY);
	LCD_FillRect(150, 289, 250, 16);
	LCD_DisplayString(0, 289, "LIGHT_GREY");

	LCD_SetColor(DARK_BLUE);
	LCD_FillRect(150, 311, 250, 16);
	LCD_DisplayString(0, 311, "DARK_BLUE");
	LCD_SetColor(DARK_GREEN);
	LCD_FillRect(150, 333, 250, 16);
	LCD_DisplayString(0, 333, "DARK_GREEN");
	LCD_SetColor(DARK_RED);
	LCD_FillRect(150, 355, 250, 16);
	LCD_DisplayString(0, 355, "DARK_RED");
	LCD_SetColor(DARK_CYAN);
	LCD_FillRect(150, 377, 250, 16);
	LCD_DisplayString(0, 377, "DARK_CYAN");
	LCD_SetColor(DARK_MAGENTA);
	LCD_FillRect(150, 399, 250, 16);
	LCD_DisplayString(0, 399, "DARK_MAGENTA");
	LCD_SetColor(DARK_YELLOW);
	LCD_FillRect(150, 421, 250, 16);
	LCD_DisplayString(0, 421, "DARK_YELLOW");
	LCD_SetColor(DARK_GREY);
	...
	{
		LCD_SetColor(LCD_GREEN - (i << 8));
		LCD_DrawLine(150 + i, 487, 150 + i, 503);
	}
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(LCD_BLUE - i);
		LCD_DrawLine(150 + i, 509, 150 + i, 525);
	}
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(LIGHT_RED - (i << 16));
		LCD_DrawLine(150 + i, 531, 150 + i, 547);
	}
	LCD_DisplayString(0, 531, "0xFF8080");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(LIGHT_GREEN - (i << 8));
		LCD_DrawLine(150 + i, 553, 150 + i, 568);
	}
	LCD_DisplayString(0, 553, "0x80FF80");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(LIGHT_BLUE - i);
		LCD_DrawLine(150 + i, 575, 150 + i, 591);
	}
	LCD_DisplayString(0, 575, "0x8080FF");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(DARK_BLUE - (i << 16));
		LCD_DrawLine(150 + i, 597, 150 + i, 613);
	}
	LCD_DisplayString(0, 597, "0x000080");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(DARK_BLUE - (i << 8));
		LCD_DrawLine(150 + i, 619, 150 + i, 635);
	}
	LCD_DisplayString(0, 619, "0x000080");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(DARK_RED - i);
		LCD_DrawLine(150 + i, 641, 150 + i, 657);
	}
	LCD_DisplayString(0, 641, "0x800000");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(DARK_RED - (i << 8));
		LCD_DrawLine(150 + i, 663, 150 + i, 679);
	}
	LCD_DisplayString(0, 663, "0x800000");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(DARK_GREEN - (i << 16));
		LCD_DrawLine(150 + i, 685, 150 + i, 701);
	}
	LCD_DisplayString(0, 685, "0x008000");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(DARK_GREEN - i);
		LCD_DrawLine(150 + i, 707, 150 + i, 723);
	}
	LCD_DisplayString(0, 707, "0x008000");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(DARK_CYAN - (i << 16));
		LCD_DrawLine(150 + i, 729, 150 + i, 745);
	}
	LCD_DisplayString(0, 729, "0x008080");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(DARK_YELLOW - i);
		LCD_DrawLine(150 + i, 751, 150 + i, 767);
	}
	LCD_DisplayString(0, 751, "0x808000");
	for (i = 0; i < 255; i++)
	{
		LCD_SetColor(DARK_MAGENTA - (i << 8));
		LCD_DrawLine(150 + i, 773, 150 + i, 789);
	}
