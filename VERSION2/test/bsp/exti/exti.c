...
void PE_EXTI_Open(void)
{
	...
	EXTI->IMR |= (LL_EXTI_LINE_10 | LL_EXTI_LINE_11); // 使能EXTI10和EXTI11中断
}
void PE_EXTI_Init(void) // for cross_cnt = 2/4/6
{
	// static u8 temp_water_cnt = 0;
	...
	LL_APB2_GRP1_EnableClock(LL_APB2_GRP1_PERIPH_SYSCFG); // 使能SYSCFG时钟,在用到外部中断时一定要用到
...
void PE_EXTI_Close(void)
{
	...
	LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_10);
	LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_11);
	EXTI->IMR &= ~(LL_EXTI_LINE_10 | LL_EXTI_LINE_11); // 失能EXTI10和EXTI11中断
}
