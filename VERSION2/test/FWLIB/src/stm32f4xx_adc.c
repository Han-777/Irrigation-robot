...
void ADC_StructInit(LEGACY_ADC_InitTypeDef* ADC_InitStruct)
{
  ...
  ADC_InitStruct->ADC_Resolution = LL_ADC_RESOLUTION_12B;
  ...
  ADC_InitStruct->ADC_DataAlign = LL_ADC_DATA_ALIGN_RIGHT;
...
void LL_ADC_CommonInit(__LL_ADC_COMMON_INSTANCE(ADC1), LEGACY_ADC_CommonInitTypeDef* ADC_CommonInitStruct)
{
  uint32_t tmpreg1 = 0;
  /* Check the parameters */
  assert_param(IS_ADC_MODE(ADC_CommonInitStruct->Multimode));
  assert_param(IS_ADC_PRESCALER(ADC_CommonInitStruct->CommonClock));
  assert_param(IS_ADC_DMA_ACCESS_MODE(ADC_CommonInitStruct->MultiDMATransfer));
  assert_param(IS_ADC_SAMPLING_DELAY(ADC_CommonInitStruct->MultiTwoSamplingDelay));
  ...
  tmpreg1 |= (uint32_t)(ADC_CommonInitStruct->Multimode | 
...
void ADC_RegularChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  if (ADC_Channel > LL_ADC_CHANNEL_9)
  {
    /* Get the old register value */
    tmpreg1 = ADCx->SMPR1;
