...
/*---------------------------- USART CR2 Configuration -----------------------*/
  ...
  tmpreg |= (uint32_t)USART_InitStruct->StopBits;
...
/*---------------------------- USART CR1 Configuration -----------------------*/
  ...
  tmpreg |= (uint32_t)USART_InitStruct->DataWidth | USART_InitStruct->Parity |
...
void LL_USART_ClockInit(USART_TypeDef* USARTx, LL_USART_ClockInitTypeDef* USART_ClockInitStruct)
{
  ...
  /* Check the parameters */
  assert_param(IS_USART_1236_PERIPH(USARTx));
  assert_param(IS_USART_CLOCK(USART_ClockInitStruct->ClockOutput));
  assert_param(IS_USART_CPOL(USART_ClockInitStruct->ClockPolarity));
  assert_param(IS_USART_CPHA(USART_ClockInitStruct->ClockPhase));
  assert_param(IS_USART_LASTBIT(USART_ClockInitStruct->LastBitClockPulse));
...
/*---------------------------- USART CR2 Configuration -----------------------*/
  ...
  tmpreg |= (uint32_t)USART_ClockInitStruct->ClockOutput | USART_ClockInitStruct->ClockPolarity | 
...
FlagStatus USART_GetFlagStatus(USART_TypeDef* USARTx, uint16_t USART_FLAG)
{
  ...
  if (USART_FLAG == LL_USART_ISR_CTS)
  {
    assert_param(IS_USART_1236_PERIPH(USARTx));
  } 
...
void USART_ClearFlag(USART_TypeDef* USARTx, uint16_t USART_FLAG)
{
  ...
  /* The CTS flag is not available for UART4 and UART5 */
  if ((USART_FLAG & LL_USART_ISR_CTS) == LL_USART_ISR_CTS)
  {
    assert_param(IS_USART_1236_PERIPH(USARTx));
  } 
