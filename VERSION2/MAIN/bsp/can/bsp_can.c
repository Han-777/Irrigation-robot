...
static void CANAddFilter(CANInstance *_instance)
{
    ...
    can_filter_conf.IdType = FDCAN_STANDARD_ID;
    can_filter_conf.FilterType = FDCAN_FILTER_MASK;
    can_filter_conf.FilterConfig = (_instance->tx_id & 1) ? FDCAN_FILTER_TO_RXFIFO0 : FDCAN_FILTER_TO_RXFIFO1; // 奇数id的模块会被分配到FIFO0,偶数id的模块会被分配到FIFO1
    can_filter_conf.FilterID1 = _instance->rx_id;
    can_filter_conf.FilterID2 = 0x7FF;
    can_filter_conf.RxBufferIndex = (_instance->can_handle == &hfdcan1) ? can1_filter_idx++ : can2_filter_idx++;
    can_filter_conf.IsCalibrationMsg = 0;
...
CANInstance *CANRegister(CAN_Init_Config_s *config)
{
    ...
    instance->txconf.IdType = FDCAN_STANDARD_ID;     // 使用标准id,扩展id则使用FDCAN_EXTENDED_ID
    ...
    instance->txconf.BitRateSwitch = FDCAN_BRS_OFF; // 不使用比特率切换
