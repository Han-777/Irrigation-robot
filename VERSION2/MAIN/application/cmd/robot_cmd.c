...
static Chassis_Upload_Data_s chassis_feed_data; // 从底盘应用接收的反馈信息信息,底盘功率枪口热量与底盘运动状态等
...
/**
 * @brief 根据灰度值计算目标clockwise flag, which will be used in target angle calculate
 */
static void ClockflagCal()
{
    get_gray_cnt();
    if (gray_cnt >= GRAY_THRESHOLD)
    {
        chassis_cmd_send.cross_cnt++;
        if (chassis_cmd_send.cross_cnt == 1 || chassis_cmd_send.cross_cnt == 2 || chassis_cmd_send.cross_cnt == 5 || chassis_cmd_send.cross_cnt == 6)
        {
            chassis_cmd_send.clockwise_rotate_flag += 1;
        }
        else if (chassis_cmd_send.cross_cnt == 3 || chassis_cmd_send.cross_cnt == 4)
        {
            chassis_cmd_send.clockwise_rotate_flag -= 1;
        }
    }
