...
void LL_DMA2D_Init(DMA2D, LL_DMA2D_InitTypeDef* DMA2D_InitStruct)
{
  ...
  assert_param(IS_DMA2D_MODE(DMA2D_InitStruct->Mode));
  assert_param(IS_DMA2D_CMODE(DMA2D_InitStruct->ColorMode));
  assert_param(IS_DMA2D_OGREEN(DMA2D_InitStruct->OutputGreen));
  assert_param(IS_DMA2D_ORED(DMA2D_InitStruct->OutputRed));
  assert_param(IS_DMA2D_OBLUE(DMA2D_InitStruct->OutputBlue));
  assert_param(IS_DMA2D_OALPHA(DMA2D_InitStruct->OutputAlpha));
  assert_param(IS_DMA2D_OUTPUT_OFFSET(DMA2D_InitStruct->LineOffset));
  assert_param(IS_DMA2D_LINE(DMA2D_InitStruct->NbrOfLines));
  assert_param(IS_DMA2D_PIXEL(DMA2D_InitStruct->NbrOfPixelsPerLines));

  /* Configures the DMA2D operation mode */
  DMA2D->CR &= (uint32_t)CR_MASK;
  DMA2D->CR |= (DMA2D_InitStruct->Mode);

  /* Configures the color mode of the output image */
  DMA2D->OPFCCR &= ~(uint32_t)DMA2D_OPFCCR_CM;
  DMA2D->OPFCCR |= (DMA2D_InitStruct->ColorMode);

  /* Configures the output color */

  if (DMA2D_InitStruct->ColorMode == LL_DMA2D_OUTPUT_MODE_ARGB8888)
  {
    outgreen = DMA2D_InitStruct->OutputGreen << 8;
    outred = DMA2D_InitStruct->OutputRed << 16;
    outalpha = DMA2D_InitStruct->OutputAlpha << 24;
  }
  else
  
    if (DMA2D_InitStruct->ColorMode == LL_DMA2D_OUTPUT_MODE_RGB888)
    {
      outgreen = DMA2D_InitStruct->OutputGreen << 8;
      outred = DMA2D_InitStruct->OutputRed << 16;
    ...
    if (DMA2D_InitStruct->ColorMode == LL_DMA2D_OUTPUT_MODE_RGB565)
    {
      outgreen = DMA2D_InitStruct->OutputGreen << 5;
      outred = DMA2D_InitStruct->OutputRed << 11;
      outalpha = (uint32_t)0x00000000;
    }

  else

    if (DMA2D_InitStruct->ColorMode == LL_DMA2D_OUTPUT_MODE_ARGB1555)
    {  
      outgreen = DMA2D_InitStruct->OutputGreen << 5;
      outred = DMA2D_InitStruct->OutputRed << 10;
      outalpha = DMA2D_InitStruct->OutputAlpha << 15;
    }

  else /* DMA2D_CMode = LL_DMA2D_OUTPUT_MODE_ARGB4444 */
  {
    outgreen = DMA2D_InitStruct->OutputGreen << 4;
    outred = DMA2D_InitStruct->OutputRed << 8;
    outalpha = DMA2D_InitStruct->OutputAlpha << 12;
  }  
  DMA2D->OCOLR |= ((outgreen) | (outred) | (DMA2D_InitStruct->OutputBlue) | (outalpha));

  /* Configures the output memory address */
  DMA2D->OMAR = (DMA2D_InitStruct->OutputMemoryAddress);

  /* Configure  the line Offset */
  DMA2D->OOR &= ~(uint32_t)DMA2D_OOR_LO;
  DMA2D->OOR |= (DMA2D_InitStruct->LineOffset);

  /* Configure the number of line and pixel per line */
  pixline = DMA2D_InitStruct->NbrOfPixelsPerLines << 16; 
  DMA2D->NLR &= ~(DMA2D_NLR_NL | DMA2D_NLR_PL);
  DMA2D->NLR |= ((DMA2D_InitStruct->NbrOfLines) | (pixline));
...
void LL_DMA2D_StructInit(LL_DMA2D_InitTypeDef* DMA2D_InitStruct)
{
  ...
  DMA2D_InitStruct->OutputGreen = 0x00;
  DMA2D_InitStruct->OutputBlue = 0x00;
  DMA2D_InitStruct->OutputRed = 0x00;
  DMA2D_InitStruct->OutputAlpha = 0x00;

  /* Initialize the output memory address */
  DMA2D_InitStruct->OutputMemoryAddress = 0x00;

  /* Initialize the output offset */
  DMA2D_InitStruct->LineOffset = 0x00;

  /* Initialize the number of line and the number of pixel per line */
  DMA2D_InitStruct->NbrOfLines = 0x00;
  DMA2D_InitStruct->NbrOfPixelsPerLines = 0x00;
}
...
void LL_DMA2D_ConfigLayer(DMA2D, LL_DMA2D_LayerCfgTypeDef* DMA2D_FG_InitStruct, 1)
{
  ...
  assert_param(IS_DMA2D_FGO(DMA2D_FG_InitStruct->LineOffset));
  assert_param(IS_DMA2D_FGCM(DMA2D_FG_InitStruct->ColorMode));
  assert_param(IS_DMA2D_FG_CLUT_CM(DMA2D_FG_InitStruct->CLUTColorMode));
  assert_param(IS_DMA2D_FG_CLUT_SIZE(DMA2D_FG_InitStruct->CLUTSize));
  assert_param(IS_DMA2D_FG_ALPHA_MODE(DMA2D_FG_InitStruct->AlphaMode));
  assert_param(IS_DMA2D_FG_ALPHA_VALUE(DMA2D_FG_InitStruct->Alpha));
  assert_param(IS_DMA2D_FGC_BLUE(DMA2D_FG_InitStruct->Blue));
  assert_param(IS_DMA2D_FGC_GREEN(DMA2D_FG_InitStruct->Green));
  assert_param(IS_DMA2D_FGC_RED(DMA2D_FG_InitStruct->Red));

  /* Configures the FG memory address */
  DMA2D->FGMAR = (DMA2D_FG_InitStruct->MemoryAddress);

  /* Configures the FG offset */
  DMA2D->FGOR &= ~(uint32_t)DMA2D_FGOR_LO;
  DMA2D->FGOR |= (DMA2D_FG_InitStruct->LineOffset);

  /* Configures foreground Pixel Format Convertor */
  DMA2D->FGPFCCR &= (uint32_t)PFCCR_MASK;
  fg_clutcolormode = DMA2D_FG_InitStruct->CLUTColorMode << 4;
  fg_clutsize = DMA2D_FG_InitStruct->CLUTSize << 8;
  fg_alpha_mode = DMA2D_FG_InitStruct->AlphaMode << 16;
  fg_alphavalue = DMA2D_FG_InitStruct->Alpha << 24;
  DMA2D->FGPFCCR |= (DMA2D_FG_InitStruct->ColorMode | fg_clutcolormode | fg_clutsize | \
                    fg_alpha_mode | fg_alphavalue);

  /* Configures foreground color */
  DMA2D->FGCOLR &= ~(DMA2D_FGCOLR_BLUE | DMA2D_FGCOLR_GREEN | DMA2D_FGCOLR_RED);
  fg_colorgreen = DMA2D_FG_InitStruct->Green << 8;
  fg_colorred = DMA2D_FG_InitStruct->Red << 16;
  DMA2D->FGCOLR |= (DMA2D_FG_InitStruct->Blue | fg_colorgreen | fg_colorred);

  /* Configures foreground CLUT memory address */
  DMA2D->FGCMAR = DMA2D_FG_InitStruct->CLUTMemoryAddress;
}
...
void LL_DMA2D_LayerCfgStructInit(LL_DMA2D_LayerCfgTypeDef* DMA2D_FG_InitStruct)
{
  ...
  DMA2D_FG_InitStruct->MemoryAddress = 0x00;

  /*!< Initialize the DMA2D foreground offset */
  DMA2D_FG_InitStruct->LineOffset = 0x00;

  /*!< Initialize the DMA2D foreground color mode */
  DMA2D_FG_InitStruct->ColorMode = LL_DMA2D_INPUT_MODE_ARGB8888;

  /*!< Initialize the DMA2D foreground CLUT color mode */
  DMA2D_FG_InitStruct->CLUTColorMode = LL_DMA2D_CLUT_COLOR_MODE_ARGB8888;

  /*!< Initialize the DMA2D foreground CLUT size */
  DMA2D_FG_InitStruct->CLUTSize = 0x00;

  /*!< Initialize the DMA2D foreground alpha mode */
  DMA2D_FG_InitStruct->AlphaMode = LL_DMA2D_ALPHA_MODE_NO_MODIF;

  /*!< Initialize the DMA2D foreground alpha value */
  DMA2D_FG_InitStruct->Alpha = 0x00;

  /*!< Initialize the DMA2D foreground blue value */
  DMA2D_FG_InitStruct->Blue = 0x00;

  /*!< Initialize the DMA2D foreground green value */
  DMA2D_FG_InitStruct->Green = 0x00;

  /*!< Initialize the DMA2D foreground red value */
  DMA2D_FG_InitStruct->Red = 0x00;

  /*!< Initialize the DMA2D foreground CLUT memory address */
  DMA2D_FG_InitStruct->CLUTMemoryAddress = 0x00;
}
...
void LL_DMA2D_ConfigLayer(DMA2D, LL_DMA2D_LayerCfgTypeDef* DMA2D_BG_InitStruct, 0)
{
  ...
  assert_param(IS_DMA2D_BGO(DMA2D_BG_InitStruct->LineOffset));
  assert_param(IS_DMA2D_BGCM(DMA2D_BG_InitStruct->ColorMode));
  assert_param(IS_DMA2D_BG_CLUT_CM(DMA2D_BG_InitStruct->CLUTColorMode));
  assert_param(IS_DMA2D_BG_CLUT_SIZE(DMA2D_BG_InitStruct->CLUTSize));
  assert_param(IS_DMA2D_BG_ALPHA_MODE(DMA2D_BG_InitStruct->AlphaMode));
  assert_param(IS_DMA2D_BG_ALPHA_VALUE(DMA2D_BG_InitStruct->Alpha));
  assert_param(IS_DMA2D_BGC_BLUE(DMA2D_BG_InitStruct->Blue));
  assert_param(IS_DMA2D_BGC_GREEN(DMA2D_BG_InitStruct->Green));
  assert_param(IS_DMA2D_BGC_RED(DMA2D_BG_InitStruct->Red));

  /* Configures the BG memory address */
  DMA2D->BGMAR = (DMA2D_BG_InitStruct->MemoryAddress);

  /* Configures the BG offset */
  DMA2D->BGOR &= ~(uint32_t)DMA2D_BGOR_LO;
  DMA2D->BGOR |= (DMA2D_BG_InitStruct->LineOffset);

  /* Configures background Pixel Format Convertor */
  DMA2D->BGPFCCR &= (uint32_t)PFCCR_MASK;
  bg_clutcolormode = DMA2D_BG_InitStruct->CLUTColorMode << 4;
  bg_clutsize = DMA2D_BG_InitStruct->CLUTSize << 8;
  bg_alpha_mode = DMA2D_BG_InitStruct->AlphaMode << 16;
  bg_alphavalue = DMA2D_BG_InitStruct->Alpha << 24;
  DMA2D->BGPFCCR |= (DMA2D_BG_InitStruct->ColorMode | bg_clutcolormode | bg_clutsize | \
                    bg_alpha_mode | bg_alphavalue);

  /* Configures background color */
  DMA2D->BGCOLR &= ~(DMA2D_BGCOLR_BLUE | DMA2D_BGCOLR_GREEN | DMA2D_BGCOLR_RED);
  bg_colorgreen = DMA2D_BG_InitStruct->Green << 8;
  bg_colorred = DMA2D_BG_InitStruct->Red << 16;
  DMA2D->BGCOLR |= (DMA2D_BG_InitStruct->Blue | bg_colorgreen | bg_colorred);
  
  /* Configures background CLUT memory address */
  DMA2D->BGCMAR = DMA2D_BG_InitStruct->CLUTMemoryAddress;
...
void LL_DMA2D_LayerCfgStructInit(LL_DMA2D_LayerCfgTypeDef* DMA2D_BG_InitStruct)
{
  ...
  DMA2D_BG_InitStruct->MemoryAddress = 0x00;

  /*!< Initialize the DMA2D background offset */
  DMA2D_BG_InitStruct->LineOffset = 0x00;

  /*!< Initialize the DMA2D background color mode */
  DMA2D_BG_InitStruct->ColorMode = LL_DMA2D_INPUT_MODE_ARGB8888;

  /*!< Initialize the DMA2D background CLUT color mode */
  DMA2D_BG_InitStruct->CLUTColorMode = LL_DMA2D_CLUT_COLOR_MODE_ARGB8888;

  /*!< Initialize the DMA2D background CLUT size */
  DMA2D_BG_InitStruct->CLUTSize = 0x00;

  /*!< Initialize the DMA2D background alpha mode */
  DMA2D_BG_InitStruct->AlphaMode = LL_DMA2D_ALPHA_MODE_NO_MODIF;

  /*!< Initialize the DMA2D background alpha value */
  DMA2D_BG_InitStruct->Alpha = 0x00;

  /*!< Initialize the DMA2D background blue value */
  DMA2D_BG_InitStruct->Blue = 0x00;

  /*!< Initialize the DMA2D background green value */
  DMA2D_BG_InitStruct->Green = 0x00;

  /*!< Initialize the DMA2D background red value */
  DMA2D_BG_InitStruct->Red = 0x00;

  /*!< Initialize the DMA2D background CLUT memory address */
  DMA2D_BG_InitStruct->CLUTMemoryAddress = 0x00;
}
