...
void DMA_Init(DMA_Stream_TypeDef* DMAy_Streamx, LL_DMA_InitTypeDef* DMA_InitStruct)
{
  ...
  /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
  assert_param(IS_DMA_CHANNEL(DMA_InitStruct->Channel));
  assert_param(IS_DMA_DIRECTION(DMA_InitStruct->Direction));
  assert_param(IS_DMA_BUFFER_SIZE(DMA_InitStruct->NbData));
  assert_param(IS_DMA_PERIPHERAL_INC_STATE(DMA_InitStruct->PeriphOrM2MSrcIncMode));
  assert_param(IS_DMA_MEMORY_INC_STATE(DMA_InitStruct->MemoryOrM2MDstIncMode));
  assert_param(IS_DMA_PERIPHERAL_DATA_SIZE(DMA_InitStruct->PeriphOrM2MSrcDataSize));
  assert_param(IS_DMA_MEMORY_DATA_SIZE(DMA_InitStruct->MemoryOrM2MDstDataSize));
  assert_param(IS_DMA_MODE(DMA_InitStruct->Mode));
  assert_param(IS_DMA_PRIORITY(DMA_InitStruct->Priority));
  assert_param(IS_DMA_FIFO_MODE_STATE(DMA_InitStruct->FIFOMode));
  assert_param(IS_DMA_FIFO_THRESHOLD(DMA_InitStruct->FIFOThreshold));
  assert_param(IS_DMA_MEMORY_BURST(DMA_InitStruct->MemBurst));
  assert_param(IS_DMA_PERIPHERAL_BURST(DMA_InitStruct->PeriphBurst));
  ...
  tmpreg |= DMA_InitStruct->Channel | DMA_InitStruct->Direction |
            ...
            DMA_InitStruct->Mode | DMA_InitStruct->Priority |
            DMA_InitStruct->MemBurst | DMA_InitStruct->PeriphBurst;
  ...
  tmpreg |= DMA_InitStruct->FIFOMode | DMA_InitStruct->FIFOThreshold;
  ...
  DMAy_Streamx->NDTR = DMA_InitStruct->NbData;

  /*------------------------- DMAy Streamx PAR Configuration -----------------*/
  /* Write to DMAy Streamx PAR */
  DMAy_Streamx->PAR = DMA_InitStruct->PeriphOrM2MSrcAddress;

  /*------------------------- DMAy Streamx M0AR Configuration ----------------*/
  /* Write to DMAy Streamx M0AR */
  DMAy_Streamx->M0AR = DMA_InitStruct->MemoryOrM2MDstAddress;
}
...
void LL_DMA_StructInit(LL_DMA_InitTypeDef* DMA_InitStruct)
{
  ...
  DMA_InitStruct->Channel = 0;

  /* Initialize the DMA_PeripheralBaseAddr member */
  DMA_InitStruct->PeriphOrM2MSrcAddress = 0;
...
{  
  ...
  if (DMA_CurrentMemory != LL_DMA_CURRENTTARGETMEM0)
  {
                           ...
                           uint32_t DMA_MemoryTarget)
{
  ...
  if (DMA_MemoryTarget != LL_DMA_CURRENTTARGETMEM0)
  {
    /* Write to DMAy Streamx M1AR */
