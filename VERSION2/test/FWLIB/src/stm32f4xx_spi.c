...
void LL_SPI_Init(SPI_TypeDef* SPIx, LL_SPI_InitTypeDef* SPI_InitStruct)
{
  ...
  assert_param(IS_SPI_DIRECTION_MODE(SPI_InitStruct->TransferDirection));
  assert_param(IS_SPI_MODE(SPI_InitStruct->Mode));
  assert_param(IS_SPI_DATASIZE(SPI_InitStruct->DataWidth));
  assert_param(IS_SPI_CPOL(SPI_InitStruct->ClockPolarity));
  assert_param(IS_SPI_CPHA(SPI_InitStruct->ClockPhase));
  assert_param(IS_SPI_NSS(SPI_InitStruct->NSS));
  assert_param(IS_SPI_BAUDRATE_PRESCALER(SPI_InitStruct->BaudRate));
  assert_param(IS_SPI_FIRST_BIT(SPI_InitStruct->BitOrder));
...
/*---------------------------- SPIx CR1 Configuration ------------------------*/
  ...
  tmpreg |= (uint16_t)((uint32_t)SPI_InitStruct->TransferDirection | SPI_InitStruct->Mode |
                  SPI_InitStruct->DataWidth | SPI_InitStruct->ClockPolarity |  
                  SPI_InitStruct->ClockPhase | SPI_InitStruct->NSS |  
                  SPI_InitStruct->BaudRate | SPI_InitStruct->BitOrder);
  /* Write to SPIx CR1 */
  SPIx->CR1 = tmpreg;
...
/*---------------------------- SPIx CRCPOLY Configuration --------------------*/
  ...
  SPIx->CRCPR = SPI_InitStruct->CRCPoly;
}
...
void LL_I2S_Init(SPI_TypeDef* SPIx, LL_I2S_InitTypeDef* I2S_InitStruct)
{
  ...
  assert_param(IS_I2S_DATA_FORMAT(I2S_InitStruct->DataFormat));
  assert_param(IS_I2S_MCLK_OUTPUT(I2S_InitStruct->MCLKOutput));
  assert_param(IS_I2S_AUDIO_FREQ(I2S_InitStruct->AudioFreq));
  assert_param(IS_I2S_CPOL(I2S_InitStruct->ClockPolarity));  
...
/*----------------------- SPIx I2SCFGR & I2SPR Configuration -----------------*/
  ...
  else
  {
    ...
    if(I2S_InitStruct->MCLKOutput == LL_I2S_MCLK_OUTPUT_ENABLE)
    {
      ...
      tmp = (uint16_t)(((((i2sclk / 256) * 10) / I2S_InitStruct->AudioFreq)) + 5);
    }
    else
    {
      /* MCLK output is disabled */
      tmp = (uint16_t)(((((i2sclk / (32 * packetlength)) *10 ) / I2S_InitStruct->AudioFreq)) + 5);
    }
  ...
  SPIx->I2SPR = (uint16_t)((uint16_t)i2sdiv | (uint16_t)(i2sodd | (uint16_t)I2S_InitStruct->MCLKOutput));
 
  /* Configure the I2S with the SPI_InitStruct values */
  tmpreg |= (uint16_t)((uint16_t)SPI_I2SCFGR_I2SMOD | (uint16_t)(I2S_InitStruct->Mode | \
                  (uint16_t)(I2S_InitStruct->Standard | (uint16_t)(I2S_InitStruct->DataFormat | \
                  (uint16_t)I2S_InitStruct->ClockPolarity))));
...
void LL_SPI_SetDataWidth(SPI_TypeDef* SPIx, uint16_t SPI_DataSize)
{
  ...
  /* Clear DFF bit */
  SPIx->CR1 &= (uint16_t)~LL_SPI_DATAWIDTH_16BIT;
