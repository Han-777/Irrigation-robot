...
void LL_GPIO_Init(GPIO_TypeDef* GPIOx, LL_GPIO_InitTypeDef* GPIO_InitStruct)
{
  ...
  for (pinpos = 0x00; pinpos < 0x10; pinpos++)
  {
    ...
    if (currentpin == pos)
    {
      ...
      GPIOx->MODER |= (((uint32_t)GPIO_InitStruct->Mode) << (pinpos * 2));
      ...
      if ((GPIO_InitStruct->Mode == LL_GPIO_MODE_OUTPUT) || (GPIO_InitStruct->Mode == LL_GPIO_MODE_ALTERNATE))
      {
        ...
        GPIOx->OSPEEDR |= ((uint32_t)(GPIO_InitStruct->Speed) << (pinpos * 2));

        /* Check Output mode parameters */
        assert_param(IS_GPIO_OTYPE(GPIO_InitStruct->OutputType));

        /* Output mode configuration*/
        GPIOx->OTYPER  &= ~((GPIO_OTYPER_OT_0) << ((uint16_t)pinpos)) ;
        GPIOx->OTYPER |= (uint16_t)(((uint16_t)GPIO_InitStruct->OutputType) << ((uint16_t)pinpos));
      }

      /* Pull-up Pull down resistor configuration*/
      GPIOx->PUPDR &= ~(GPIO_PUPDR_PUPDR0 << ((uint16_t)pinpos * 2));
      GPIOx->PUPDR |= (((uint32_t)GPIO_InitStruct->Pull) << (pinpos * 2));
    }
  }
}
...
uint8_t LL_GPIO_IsOutputPinSet(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
  ...
  if (((GPIOx->ODR) & GPIO_Pin) != (uint32_t)0)
  {
    bitstatus = (uint8_t)1;
  }
  else
